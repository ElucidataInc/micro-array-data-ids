...
REPO_ID = "1610017790779"
LAG_DAYS = 2
pdat = (datetime.today() - timedelta(days=LAG_DAYS)).strftime("%Y/%m/%d")

search_url  = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&term="Expression+Profiling+by+High+Throughput+Sequencing"[DataSet Type]+AND+("single+cell"[Title]+OR+"single+cell"[Description])+AND+("Mus+musculus"[Organism]+OR+"Homo+sapiens"[Organism]+OR+"rattus norvegicus"[Organism])+AND+"{pdat}"[PDAT]&usehistory=y'
